...
If your studio uses mapped drives (like `S:\`), RenderFlow can help bridge the gap with its [Mapped Drives](/products/renderflow/getting-started/windows-network-setup) feature, but using UNC paths directly is the better long-term practice.
...
This is one area where RenderFlow helps significantly. It scans every connected machine for installed software and versions, then automatically checks whether each node can handle a given job before assigning it. Incompatible nodes are flagged and excluded. The [Software Analytics](/products/renderflow/software-analytics/overview) view lets you compare versions across your entire farm at a glance.
...
- **Cloud bursting**: when your local farm isn't enough, scale to [RenderFlow Cloud](/products/renderflow/cloud-rendering/overview) without changing your workflow

## What about cloud render farms?

If you don't want to build or maintain your own hardware, [RenderFlow Cloud](/products/renderflow/cloud-rendering/overview) provides access to powerful cloud render nodes directly from the RenderFlow interface. Assets are uploaded automatically, jobs appear in the same list as local renders, and rendered frames are delivered back to your local path. No separate portal, no separate tools.
