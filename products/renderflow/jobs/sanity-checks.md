...
Sanity checks are automatic validations that run on your scene when you select a file in the [Submitter](/products/renderflow/jobs/submitter). They catch common mistakes that would cause a render to fail or produce unexpected results on the farm.
...
For full details on managing, editing, and creating sanity checks, see the dedicated [Sanity Check](/products/renderflow/sanity-check/overview) section.
