...
**Reserve** sets the node to a reserved state. Reserved nodes do not pick up regular jobs. This is used primarily for [Spawner Mode](/products/renderflow/nodes/spawner-mode).

**Pool** reassigns the node to a different pool.

**CPU Affinity** controls how many CPU cores the node uses for rendering. See [CPU Affinity](/products/renderflow/nodes/cpu-affinity).

**Edit Tags** opens the tag editor for the selected nodes. See [Tags](/products/renderflow/nodes/tags).
...
**Start Spawner** and **Stop Spawner** control [Spawner Mode](/products/renderflow/nodes/spawner-mode) for distributed rendering.
...
<Frame caption="Node context menu with batch commands">
  <img src="https://placehold.co/900x500?text=Node+context+menu" alt="Right-click context menu showing status, pool, restart, shutdown, and other commands" />
</Frame>
