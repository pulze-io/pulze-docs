...
Because of how Corona DR works internally, RenderFlow cannot directly assign specific render nodes to a DR session (beyond the master). Instead, participating nodes need to be in [Spawner Mode](/products/renderflow/nodes/spawner-mode). You can enable this on selected nodes from the Nodes context menu, or automate it with the [Scheduler](/products/renderflow/scheduler/overview).

## Submission

The submission properties are the same as a standard [3ds Max](/products/renderflow/job-types/3ds-max) render job. Select the **Corona DR** template, pick your file, and configure settings as usual.
