...
3ds Max is the most mature integration in RenderFlow. It supports still image, tiled, and animation rendering, with full scene property detection, asset scanning, [sanity checks](/products/renderflow/jobs/sanity-checks), and [automatic requirements](/products/renderflow/jobs/requirements).
...
**From 3ds Max**, click the RenderFlow button in the toolbar. The [Submitter](/products/renderflow/jobs/submitter) opens with your current scene already selected and all properties pre-filled.
...
<Frame caption="Submitting a 3ds Max job from the RenderFlow app">
  <img src="https://placehold.co/900x500?text=3ds+Max+Submitter" alt="3ds Max job submitter showing scene properties and job settings" />
</Frame>
...
For the full list of supported 3ds Max versions, render engines, and plugins, see [Supported Apps](/products/renderflow/getting-started/supported-apps).

## Common workflow

The typical 3ds Max workflow is straightforward: set up your scene, make sure all assets are on [shared storage](/products/renderflow/getting-started/windows-network-setup) using UNC paths, open the Submitter, review the properties and sanity check results, adjust job settings if needed, and submit. Available nodes with the correct software will pick up the job automatically.
