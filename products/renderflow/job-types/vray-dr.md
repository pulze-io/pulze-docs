...
V-Ray Distributed Rendering (DR) allows multiple computers to work together on a single still image, similar to [Corona DR](/products/renderflow/job-types/corona-dr). The key difference is that V-Ray DR in RenderFlow is **fully managed**. You do not need to start Spawner Mode or launch V-Ray Spawner manually on any node.
...
The submission properties are the same as a standard [3ds Max](/products/renderflow/job-types/3ds-max) render job. Select the **V-Ray DR** template, pick your file, and configure settings as usual.
