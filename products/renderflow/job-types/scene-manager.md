...
Select the **Scene Manager** template in the [Submitter](/products/renderflow/jobs/submitter), then select or browse a .max file that contains Scene Manager setups. Once the scene information is read, the Submitter switches to the multi-job view.
...
<Frame caption="Multi-job submitter view with scene list on the left and properties on the right">
  <img src="https://placehold.co/900x500?text=Scene+Manager+multi-job+submitter" alt="Scene Manager submitter showing multiple jobs and batch settings" />
</Frame>
